...
| # | 항목 | 4차시 판 | 지금 | 이유 |
|--:|---|---|---|---|
| 1 | event 13 공진 · τ · Q | 521 Hz / 2.07 ms / 3.4 | — (공진 미검출) | 500 Hz 하한 위 첫 FFT 칸이 뽑힌 값, 파형에 울림 없음 (§2-1) |
| 2 | "걸음과 공진이 1,666배 차이" | 절 제목 + 비율 표 | 삭제 | 걸음 2 Hz는 측정값이 아니고(이 측정의 FFT는 104 Hz 아래를 보지 못한다), 두 주파수의 비가 출력 손실 배율을 뜻하지도 않는다. "발걸음은 공진보다 훨씬 느린 준정적 입력"이라는 결론만 §7로 이어진다 |
| 3 | R_opt 계산표 | 3.2 kΩ / 5.3 MΩ | P–R 실측으로 결정 | 사인파 가정의 식 + 가정한 C_p (§2-3) |
| 4 | 추정 힘 F (d₃₃ = 400 pC/N) | 28–85 N | 삭제 | 굽힘(d₃₁) 모드 소자, 링잉 피크는 정적 전하가 아님 (§2-4) |
